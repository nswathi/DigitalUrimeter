/*
    This is an edited version of the LCD library from Adafruit
*/
...
void Adafruit_LiquidCrystal::write2(uint8_t value) {
    send(value, HIGH);
}

void Adafruit_LiquidCrystal::print2(const char * str) {
    //const char * ch = str;
    while (*str != '\0') write2(*str++);
}
...
void Adafruit_LiquidCrystal::write4bits(uint8_t value) {
    ...
    out = _i2c.readGPIO(); //read current gpio

    // speed up for i2c since its sluggish
    for (int i = 0; i < 4; i++) {
        out &= ~_BV(_data_pins[i]); //turn off data pins
        out |= ((value >> i) & 0x1) << _data_pins[i]; //set bits to write
    }
